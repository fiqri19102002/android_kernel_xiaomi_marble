...
static struct xiaomi_touch_interface *interfaces;
...
int register_xiaomi_touch_client(struct xiaomi_touch_interface *interface)
{
	if (interfaces)
		return -EINVAL;
...
int unregister_xiaomi_touch_client(void)
{
	interfaces = NULL;
...
static int touch_mode_get(enum touch_mode mode)
{
	...
	// Nonui mode are not passed down to the touch driver
	if (mode == TOUCH_MODE_NONUI_MODE)
...
static void register_panel_handler(struct work_struct *work)
{
	...
	panel = touch_register_panel_event_notifier(
		node, "panel", PANEL_EVENT_NOTIFICATION_PRIMARY,
		PANEL_EVENT_NOTIFIER_CLIENT_XIAOMI_TOUCH,
		&panel_cookie);
	if (!panel) {
